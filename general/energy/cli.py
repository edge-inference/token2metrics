...
from energy.aggregate import aggregate_energy_metrics, generate_detailed_model_summary
from energy.correlate import EnergyPerformanceCorrelator
from energy.insights import PowerInsightsAnalyzer
...
def run_energy_analysis(base_dir: str, verbose: bool = False) -> None:
    """Run basic energy analysis."""
    print("🔋 Energy Analysis Pipeline")
    print("="*50)
    
    base_dir = _resolve_existing_dir(base_dir)
    if verbose:
        print(f"Base directory: {base_dir}")
    
    # Generate detailed model summary with raw data
    model_summary = generate_detailed_model_summary(base_dir)
...
def run_correlation_analysis(energy_dir: str, performance_file: str, verbose: bool = False) -> None:
    ...
    energy_dir = _resolve_existing_dir(energy_dir)
    performance_file = _resolve_existing_dir(performance_file)
    if verbose:
        print(f"Energy directory: {energy_dir}")
        print(f"Performance file: {performance_file}")
    
    # Check if files exist (after resolution)
...
def run_insights_analysis(correlation_file: str = None, verbose: bool = False) -> None:
    """Run power insights analysis using correlation file."""
    ...
    if not correlation_file:
        ...
        if not output_dir.exists():
            print("❌ Error: energy_results directory not found")
...
def run_fitting_analysis(correlation_file: str = None, output_dir: str = "results/fitting", 
    ...
    # Auto-detect correlation file if not provided
    if not correlation_file:
        energy_results_dir = Path('./energy_results')
...
Examples:
  ...
  python -m energy.cli --base-dir ./raw/tegra
  
  # Energy-performance correlation
  python -m energy.cli --correlate --energy-dir ./raw/tegra 
  ...
  python -m energy.cli --correlate --verbose
    ...
    parser.add_argument(
        '--base-dir',
        default='./raw/tegra',
        help='Base directory containing energy CSV files (default: ./raw/tegra)'
    )
    ...
    parser.add_argument(
        '--energy-dir',
        default='./raw/tegra',
        help='Directory containing energy CSV files for correlation (default: ./raw/tegra)'
    )
    ...
    try:
        if args.insights:
            ...
            if not args.performance_file:
                # Priority 1 → Full-MMLU workbook in datasets/tokens/
                mmlu_candidates = [
                    Path('datasets/tokens/full_mmlu_by_model_tegra.xlsx'),
                    Path('../datasets/tokens/full_mmlu_by_model_tegra.xlsx')
                ]
                for cand in mmlu_candidates:
                    if cand.exists():
                        args.performance_file = str(cand)
                        print(f"🔍 Auto-detected performance file: {args.performance_file}")
                        break

                if not args.performance_file:
                    # Priority 2 → latest results under ./processed_results/
                ...
                if not args.performance_file:
                    print("❌ Error: No performance file specified and none found in default locations")
            ...
            # Run basic energy analysis
            run_energy_analysis(args.base_dir, args.verbose)

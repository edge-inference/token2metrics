...
xlsx_candidates = list((script_dir / "datasets/prefill/processed_results").glob("all_results_by_model*.xlsx"))
if not xlsx_candidates:
    raise FileNotFoundError(
        "No matching XLSX file found in processed_results/ (searched from script root)")
...
def plot_combined(coeffs_dict, xls):
    ...
    ax.set_xlim([0, 650])
    x_ticks = np.arange(0, 651, 128)
    ax.set_xticks(x_ticks)
    ax.legend()
    fig.tight_layout()
    out_path = Path("plots") / "combined_prefill_fit.pdf"
...
def plot_combined_1_5B(coeffs_dict, xls):
    ...
    ax.legend()
    fig.tight_layout()
    out_path = Path("plots") / "combined_1_5B_prefill_fit.pdf"
    fig.savefig(out_path)
    plt.close(fig)

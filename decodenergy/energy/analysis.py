...
class PowerAnalyzer:
    ...
    def filter_questions_by_token_ranges(self) -> pd.DataFrame:
        ...
        for model_name, df in self.model_data.items():
            df_copy = df.copy()
            df_copy['target_token_range'] = df_copy['output_tokens']
    ...
    def generate_power_analysis(self) -> pd.DataFrame:
        ...
        analysis_groups = self.filtered_data.groupby(['model_name', 'output_tokens'])
        analysis_results = []
        for (model, output_tokens), group in analysis_groups:
            tokens_per_second = group['tokens_per_second']
            result = {
                'model_name': model,
                'target_token_range': output_tokens,

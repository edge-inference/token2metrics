...
class IndividualPlotter(BasePlotter):
    ...
    def _create_avg_power_plot(self, analysis_df: pd.DataFrame) -> str:
        ...
        ax.set_xlabel('Output Length', fontsize=8)
        ax.set_ylabel('Average Power (W)', fontsize=8)
        ax.legend(title='Model', fontsize=6, title_fontsize=6, loc='upper left', bbox_to_anchor=(0.02, 0.98))
    ...
    def _create_energy_efficiency_plot(self, analysis_df: pd.DataFrame) -> str:
        ...
        ax.set_xlabel('Output Length', fontsize=8)
        ax.set_ylabel('Energy (J/token)', fontsize=8)
        ax.legend(title='Model', fontsize=6, title_fontsize=6, loc='upper right', bbox_to_anchor=(0.98, 0.98))
        ax.tick_params(axis='both', which='major', labelsize=6)
        plt.tight_layout()
    ...
    def _create_tokens_per_second_plot(self, analysis_df: pd.DataFrame) -> str:
        ...
        ax.set_xlabel('Output Length', fontsize=8)
        ax.set_ylabel('Output Tokens per Second', fontsize=8)
        ax.legend(title='Model', fontsize=6, title_fontsize=6, loc='upper right', bbox_to_anchor=(0.98, 0.98))
        ax.tick_params(axis='both', which='major', labelsize=6)
        plt.tight_layout()
